...
var MfunrkdocAdd = function() {

	var userAddFormFun = function() {
		var userAddForm = $("#mfunrkdocAddForm");
		// 字符验证 
		userAddForm.validate({
			...
			submitHandler : function(form) {
				...
				var rkdjNo = $("input[name='rkdjNo']").val();
				var rkdjComeinfo = $("input[name='rkdjComeinfo']").val();
				var docsStr = $('#tags').val();
				$.ajax({
					url : getRootPath() + "/mfunrkdoc/insertDocAndDocs",
					type : 'post',
					data : {
						"arehouse_id":arehouse_id,
						"rkdjNo" : rkdjNo,
						"rkdjComeinfo" : rkdjComeinfo,
						"docsStr" : docsStr
					},
...
//                console.log('删除了'+tag)
	...
	$("button.addTags").on("click",function(){
		...
		if(goodsName==''||goodsCount==''||goodsDw==''||goodsDw==undefined){
			...
			var strarry = goodsName+"-"+goodsCount;
			//name dw no count
