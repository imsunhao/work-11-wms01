/*!
 * jQuery-region v1.0
 * https://www.infchd.com
 * Copyright 2016 xuechao
 * <!-- 仓库区域平面图 jquery插件实现图形化管理仓库中区域包含的储位信息 参考 seatCharts -->
 */
...
(function($) {
	...
	$.fn.seatCharts = function(setup) {
		...
		//如果有seatcharts对象与当前元素有关则返回它
		if (this.data('seatCharts')) {
			return this.data('seatCharts');
		}

		var fn = this, seats = {}, seatIds = [], legend, settings = {
			animate : false, //需要jquery ui
			naming : {
				top : true,
				left : true,
		...
		//当我们绘制map的时候，seat 将生成基本的座位对象
		seat = (function(seatCharts, seatChartsSettings) {
			return function(setup) {
				var fn = this;

				fn.settings = $.extend({
					status : 'available', //available 可用, unavailable 不可用, selected 选择
					style : 'available',
					data : seatChartsSettings.seats[setup.character] || {} //如果用户没有做个任何设置，生成一个空的hash数据
				}, setup);
